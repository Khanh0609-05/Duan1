 ...
 */
import Repo.SanPhamChiTietRepo;
import duan1_bangiay.model.SanPhamChiTietModel;
import duan1_bangiay.model.ModelKichThuoc;
import Repo.KichThuocRepo;
import duan1_bangiay.model.ModelMauSac;
...
public class SanPhamView extends javax.swing.JFrame {
    ...
    private ThuongHieuRepo rp = new ThuongHieuRepo();
    private MauSacRepo rpMauSac = new MauSacRepo();
    private KichThuocRepo rpKichThuoc = new KichThuocRepo();
    private SanPhamChiTietRepo rpCTSP = new SanPhamChiTietRepo();
    ...
    // Biến toàn cục cho bộ nhớ đệm và cờ
    private List<ModelThuongHieu> cachedTH;
    private List<ModelMauSac> cachedMS;
    private List<ModelKichThuoc> cachedKT;
    private boolean isComboBoxDataLoaded = false;
    ...
    private void loadComboBoxData() {
        ...
        if (cachedTH == null) {
            cachedTH = rp.getallTH();
        }
        if (cachedTH != null && !cachedTH.isEmpty()) {
            for (ModelThuongHieu th : cachedTH) {
        ...
        if (cachedMS == null) {
            cachedMS = rpMauSac.getAllMS();
        }
        if (cachedMS != null && !cachedMS.isEmpty()) {
            for (ModelMauSac ms : cachedMS) {
        ...
        if (cachedKT == null) {
            cachedKT = rpKichThuoc.getAllKT();
        }
        if (cachedKT != null && !cachedKT.isEmpty()) {
            for (ModelKichThuoc kt : cachedKT) {
        ...
        isComboBoxDataLoaded = true; // Đánh dấu đã tải
    }
...
// Xóa bộ nhớ đệm
    ...
    private void btn_themMSActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btn_themMSActionPerformed
        ...
        if (ms != null) {
            // Sinh mã mới và cập nhật
            String newMaMS = generateNewMaMS();
            ms.setMa(newMaMS); // Cập nhật mã mới
            ms.setTrangThai(true); // Trạng thái Hoạt động
            txt_maMS.setText(newMaMS); // Hiển thị mã trên giao diện
            ...
            if (rpMauSac.checkTrungMS(ms.getTen()) != null) {
                ...
                if (rpMauSac.themMS(ms) > 0) {
                    ...
                    // Cập nhật bảng và combo box
                    fillTableMauSac(rpMauSac.getAllMS());
                    clearCache(); // Xóa bộ nhớ đệm
                    isComboBoxDataLoaded = false; // Cho phép tải lại combo box
                    loadComboBoxData(); // Làm mới combo box
                    clearFormMS(); // Xóa form
                } else {
    ...
    private void btn_suaMSActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btn_suaMSActionPerformed
        ...
        if (ms != null) {
            ...
            // Kiểm tra trùng tên màu sắc (nếu cần)
            ModelMauSac existing = rpMauSac.checkTrungMS(ms.getTen());
            if (existing != null && !existing.getMa().equals(maCu)) {
                JOptionPane.showMessageDialog(this, "Tên màu sắc đã tồn tại");
                return;
            }

            // Lấy idMauSac từ bộ nhớ đệm nếu có
            int id = -1;
            if (cachedMS != null) {
                id = cachedMS.stream()
                        .filter(m -> m.getMa().equals(maCu))
                        .findFirst()
                        .map(ModelMauSac::getIdMauSac)
                        .orElse(-1);
            }
            if (id == -1) {
                // Nếu không có trong bộ nhớ đệm, truy vấn cơ sở dữ liệu
                ModelMauSac found = rpMauSac.getAllMS().stream()
                        .filter(m -> m.getMa().equals(maCu))
                        .findFirst()
                        .orElse(null);
                if (found != null) {
                    id = found.getIdMauSac();
                }
            }

            if (id == -1) {
                JOptionPane.showMessageDialog(this, "Không tìm thấy màu sắc với mã " + maCu);
                return;
            }
    ...
    private void btn_themKTActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btn_themKTActionPerformed
        ...
        if (kt != null) {
            ...
            kt.setMa(newMaKT); // Cập nhật mã mới
            kt.setTrangThai(true); // Trạng thái Hoạt động
            txt_maKT.setText(newMaKT); // Hiển thị mã trên giao diện
    ...
    private void btn_suaKTActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btn_suaKTActionPerformed
        ...
        if (kt != null) {
            ...
            // Kiểm tra trùng tên kích thước
            ModelKichThuoc existing = rpKichThuoc.checkTrungKT(kt.getTen());
            if (existing != null && !existing.getMa().equals(maCu)) {
                JOptionPane.showMessageDialog(this, "Tên kích thước đã tồn tại");
                return;
            }

            // Lấy id từ bộ nhớ đệm nếu có
            int id = -1;
            if (cachedKT != null) {
                id = cachedKT.stream()
                        .filter(k -> k.getMa().equals(maCu))
                        .findFirst()
                        .map(ModelKichThuoc::getId)
                        .orElse(-1);
            }
            if (id == -1) {
                // Nếu không có trong bộ nhớ đệm, truy vấn cơ sở dữ liệu
                ModelKichThuoc found = rpKichThuoc.getAllKT().stream()
                        .filter(k -> k.getMa().equals(maCu))
                        .findFirst()
                        .orElse(null);
                if (found != null) {
            ...
            if (id == -1) {
                JOptionPane.showMessageDialog(this, "Không tìm thấy kích thước với mã " + maCu);
                return;
            }
    ...
    private void btn_themspctActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btn_themspctActionPerformed
        ...
        try {
            ...
            try {
                ...
                if (donGia.compareTo(new BigDecimal("0")) <= 0) {
                    JOptionPane.showMessageDialog(this, "Đơn giá phải lớn hơn 0 VND");
                    txt_dongiaspct.requestFocus();
                    return;
                }
            ...
            if (isInserted) {
                ...
                ArrayList<SanPham.Model.SanPhamChiTietModel> list = repository.getAllSPCT();
                DefaultTableModel model = (DefaultTableModel) tbl_spct.getModel();
                model.setRowCount(0);
                for (SanPham.Model.SanPhamChiTietModel ctsp : list) {
                    model.addRow(ctsp.toDataRowSPCT());
                }
    ...
    private void btn_ngungbanSPctActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btn_ngungbanSPctActionPerformed
        ...
        try {
            ...
            SanPhamChiTietRepo repository = new SanPhamChiTietRepo();
    ...
    private void btn_suaTHActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btn_suaTHActionPerformed
        int i = tbl_thuonghieu.getSelectedRow();
        if (i == -1) {
    ...
    private void btn_themTHActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btn_themTHActionPerformed
        ...
        if (th != null) {
            ...
            th.setTrangThai(true); // Trạng thái Hoạt động
            th.setMaTH(newMaTH); // Cập nhật mã mới
            txt_maTH.setText(newMaTH); // Hiển thị mã trên giao diện
    ...
    private void tbl_spctMouseEntered(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_tbl_spctMouseEntered
        // TODO add your handling code here:
    }//GEN-LAST:event_tbl_spctMouseEntered
...
// Xóa dữ liệu form
    ...
    private ModelKichThuoc readFormKT() {
        ...
        if (!tenKT.matches("[0-9.]+")) {
            JOptionPane.showMessageDialog(this, "Tên kích thước chỉ được chứa số và dấu chấm.");
            txt_tenKT.requestFocus();
            return null;
        }

        // Kiểm tra nếu tenKT là số (số nguyên hoặc thập phân với dấu chấm)
        if (tenKT.matches("\\d+(\\.\\d{1,2})?")) {
            // Kiểm tra không bắt đầu bằng 0
            if (tenKT.startsWith("0") && !tenKT.equals("0")) {
                JOptionPane.showMessageDialog(this, "Kích thước số không được bắt đầu bằng 0");
                txt_tenKT.requestFocus();
                return null;
            }
            try {
                double tenKTValue = Double.parseDouble(tenKT);
                if (tenKTValue <= 0) {
                    JOptionPane.showMessageDialog(this, "Kích thước không được bằng hoặc nhỏ hơn 0");
                    txt_tenKT.requestFocus();
                    return null;
                }
            ...
            // Nếu không phải số hợp lệ (ví dụ: chứa nhiều dấu chấm)
            JOptionPane.showMessageDialog(this, "Tên kích thước phải là số hợp lệ (tối đa 2 chữ số thập phân)");
            txt_tenKT.requestFocus();
            return null;
        }
