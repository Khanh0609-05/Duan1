...
public class NhanVienView extends javax.swing.JPanel {
    ...
    private void btn_suaActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btn_suaActionPerformed
        ...
        if (kiemTra1()) {
            try {
                ...
                if (nv.getIdChucVu() == 1 && !nv.isTrangThai()) {
                    int activeManagers = repo.countActiveManagers(nv.getId());
                    if (activeManagers < 1) {
                        JOptionPane.showMessageDialog(this, "Không thể sửa trạng thái của quản lý này thành 'Đã nghỉ' vì phải có ít nhất một quản lý đang làm việc!", "Lỗi", JOptionPane.ERROR_MESSAGE);
                        return;
                    }
                }

                // Kiểm tra trước khi cập nhật: xem có phải đang sửa tài khoản của chính mình không
                boolean isSelfEdit = (nv.getId() == UserSession.getInstance().getUserId());
                boolean isChangingToInactive = !nv.isTrangThai(); // false = đã nghỉ (0)

                int result = repo.updateNV(nv);
                if (result > 0) {
                    JOptionPane.showMessageDialog(this, "Cập nhật thành công");

                    // Nếu đang sửa chính mình và thay đổi trạng thái thành "đã nghỉ" (0)
                    if (isSelfEdit && isChangingToInactive) {
                        JOptionPane.showMessageDialog(this,
                                "Bạn đã thay đổi trạng thái của mình thành 'Đã nghỉ'. Hệ thống sẽ đăng xuất.",
                                "Thông báo", JOptionPane.INFORMATION_MESSAGE);

                        // Sử dụng Timer để cho phép người dùng thấy thông báo trước khi đăng xuất
                        ...
                        return; // Thoát khỏi phương thức vì người dùng sẽ bị đăng xuất
                    } else if (isSelfEdit) {
                        // Cập nhật trạng thái trong session nếu người dùng thay đổi thông tin của mình
                        int statusValue = nv.isTrangThai() ? 1 : 0;
                        UserSession.getInstance().updateUserStatus(statusValue);
                    }
    ...
    try {
        // Kiểm tra nếu nhân viên là quản lý
        if (chucVu.equalsIgnoreCase("Quản lý")) {
            int activeManagers = repo.countActiveManagers(Integer.parseInt(idNhanVien)); // Loại trừ nhân viên hiện tại
            if (activeManagers < 1) { // Đảm bảo vẫn còn ít nhất 1 quản lý khác đang làm việc
                JOptionPane.showMessageDialog(this, "Không thể cập nhật trạng thái của quản lý này thành 'Đã nghỉ' vì phải có ít nhất một quản lý đang làm việc!", "Lỗi", JOptionPane.ERROR_MESSAGE);
                return;
            }
        }
        ...
        JOptionPane.showMessageDialog(this, "Lỗi khi kiểm tra trạng thái quản lý: " + e.getMessage());
        e.printStackTrace();
    }
