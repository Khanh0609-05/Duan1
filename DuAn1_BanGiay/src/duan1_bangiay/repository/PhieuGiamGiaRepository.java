...
public class PhieuGiamGiaRepository {
    ...
    public int xoaPGG(String id) {
        ...
        try {
            PreparedStatement ps = this.conn.prepareStatement(sql);
            ps.setInt(1, Integer.parseInt(id)); // Chuyển id thành số nguyên và gán vào tham số
    ...
    public ArrayList<PhieuGiamGia> search(String search) {
        ...
        try (PreparedStatement ps = conn.prepareStatement(sql)) {
            // Gán giá trị cho các tham số (tất cả đều tìm kiếm kiểu "chứa" trừ MaPhieuGiamGia)
            ps.setString(1, "%" + search + "%");  // ID
            ps.setString(2, search + "%");         // MaPhieuGiamGia (bắt đầu bằng)
            ps.setString(3, "%" + search + "%");  // TenPhieuGiamGia
            ps.setString(4, "%" + search + "%");  // KieuGiam (true/false)
            ps.setString(5, "%" + search + "%");  // MucGiam
            ps.setString(6, "%" + search + "%");  // MucGiamToiDa
            ps.setString(7, "%" + search + "%");  // HoaDonToiThieu
            ps.setString(8, "%" + search + "%");  // NgayBatDau (định dạng yyyy-MM-dd)
            ps.setString(9, "%" + search + "%");  // NgayKetThuc (định dạng yyyy-MM-dd)
            ps.setString(10, "%" + search + "%"); // SoLuong
            ps.setString(11, "%" + search + "%"); // DaDung
            ps.setString(12, "%" + search + "%"); // TrangThai (true/false)
