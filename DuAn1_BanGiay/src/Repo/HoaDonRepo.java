...
"                LEFT JOIN PhieuGiamGia ON HoaDon.IDPhieuGiamGia = PhieuGiamGia.ID\n" +
"                WHERE HoaDon.TrangThai = 1\n" +
"                ORDER BY HoaDon.ThanhTien DESC;";
        try {
    ...
    public HoaDon getHoaDonByMa(String ma) {
        HoaDon hoaDon = null;
        String sql = "SELECT HoaDon.MaHoaDon, KhachHang.MaKhachHang, NhanVien.MaNhanVien, PhieuGiamGia.MaPhieuGiamGia, "
                + "HoaDon.NgayTao, HoaDon.TongTien, HoaDon.GiamGia, HoaDon.ThanhTien "
                + "FROM HoaDon "
                + "LEFT JOIN KhachHang ON HoaDon.IDKhachHang = KhachHang.ID "
                + "LEFT JOIN NhanVien ON HoaDon.IDNhanVien = NhanVien.ID "
                + "LEFT JOIN PhieuGiamGia ON HoaDon.IDPhieuGiamGia = PhieuGiamGia.ID "
                + "WHERE HoaDon.MaHoaDon = ? AND HoaDon.TrangThai = 1";
        ...
        try {
            ...
            if (rs.next()) {
                ...
                String maPhieuGiamGia = rs.getString(4);
                java.sql.Timestamp timestamp = rs.getTimestamp(5);
                LocalDateTime ngayTao = (timestamp != null) ? timestamp.toLocalDateTime() : null;
                double tongTien = rs.getDouble(6);
                double giamGia = rs.getDouble(7);
                double thanhTien = rs.getDouble(8);

                hoaDon = new HoaDon(maHoaDon, maKhachHang, maNhanVien, maPhieuGiamGia, ngayTao, tongTien, giamGia, thanhTien);
            }
        } catch (Exception e) {
            e.printStackTrace();
        } finally {
