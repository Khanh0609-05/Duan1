...
public class PhieuGiamGiaRepo {
    ...
    public PhieuGiamGia getPhieuGiamGiaByMa(String maPhieuGiamGia) {
        String sql = "SELECT ID, MaPhieuGiamGia, TenPhieuGiamGia, kieuGiam, mucGiam, mucGiamToiDa, "
                + "hoaDonToiThieu, soLuong, daDung, ngayBatDau, ngayKetThuc, trangThai "
                + "FROM PhieuGiamGia WHERE MaPhieuGiamGia = ?";
        try (Connection conn = DBConnect.getConnection(); PreparedStatement ps = conn.prepareStatement(sql)) {
            ps.setString(1, maPhieuGiamGia);
            try (ResultSet rs = ps.executeQuery()) {
                if (rs.next()) {
                    PhieuGiamGia phieuGiamGia = new PhieuGiamGia();
                    phieuGiamGia.setId(rs.getInt("ID"));
                    phieuGiamGia.setMaPhieuGiamGia(rs.getString("MaPhieuGiamGia"));
                    phieuGiamGia.setTenPhieuGiamGia(rs.getString("TenPhieuGiamGia"));
                    String kieuGiamStr = rs.getString("kieuGiam");
                    phieuGiamGia.setKieuGiam(kieuGiamStr != null && kieuGiamStr.equals("%"));
                    phieuGiamGia.setMucGiam(rs.getDouble("mucGiam"));
                    phieuGiamGia.setMucGiamToiDa(rs.getDouble("mucGiamToiDa"));
                    phieuGiamGia.setHoaDonToiThieu(rs.getDouble("hoaDonToiThieu"));
                    phieuGiamGia.setSoLuong(rs.getInt("soLuong"));
                    phieuGiamGia.setDaDung(rs.getInt("daDung"));
                    phieuGiamGia.setNgayBatDau(rs.getDate("ngayBatDau"));
                    phieuGiamGia.setNgayKetThuc(rs.getDate("ngayKetThuc"));
                    phieuGiamGia.setTrangThai(rs.getBoolean("trangThai"));
                    return phieuGiamGia;
                }
            }
        } catch (SQLException e) {
